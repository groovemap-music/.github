...
if (!designRoot) {
  throw new Error("GROOVEMAP_DESIGN_REPO must point to the clean checkout of the pinned design commit");
}
...
execFileSync(process.execPath, [path.join(designRoot, "brand", "render.mjs"), "--check"], {
  stdio: "inherit",
});

for (const asset of promotedAssets) {
  const sourceBytes = await readFile(path.join(designRoot, asset.source));
  const sourceDigest = createHash("sha256").update(sourceBytes).digest("hex");
  if (sourceDigest !== asset.sha256) {
    throw new Error(`${asset.source} does not match its reviewed design digest`);
  }
}

for (const asset of promotedAssets) {
  await copyFile(path.join(designRoot, asset.source), path.join(repositoryRoot, asset.destination));
}

const checksums = promotedAssets
  .map((asset) => `${asset.sha256}  ${path.basename(asset.destination)}`)
  .join("\n");
await writeFile(path.join(repositoryRoot, "profile", "assets", "checksums.sha256"), `${checksums}\n`, "utf8");
...
await writeFile(
  path.join(repositoryRoot, "profile", "assets", "provenance.json"),
